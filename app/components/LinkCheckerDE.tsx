...
export default function LinkCheckerDE() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Fehler beim Prüfen der Links');
      }
    ...
    <div className="bg-gradient-to-b from-slate-800 to-slate-900 py-16" id="free-checker">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold text-center mb-4 text-white">
          Prüfe deine Links jetzt — Kostenlos
        </h2>
        <p className="text-xl text-center text-gray-200 mb-8">
          Sieh in 30 Sekunden, wie viele kaputte Links du hast
        </p>
        ...
        <div className="bg-slate-700 rounded-lg shadow-xl p-8 border-2 border-slate-600">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? 'Scanne...' : 'Meine Links scannen (Kostenlos) →'}
            </button>
          </form>
          ...
          {loading && (
            <div className="mt-8 text-center">
              ...
              <p className="text-gray-200">
                Scanne dein Video nach Affiliate-Links...
              </p>
            </div>
          )}
                ...
                <>
                  <div className="space-y-3 mb-6">
                    <div className="flex justify-between items-center text-lg">
                      <span className="text-gray-200 font-medium">✅ Funktionierende Links:</span>
                      <strong className="text-green-400 text-2xl">{result.working}</strong>
                    </div>
                    <div className="flex justify-between items-center text-lg">
                      <span className="text-gray-200 font-medium">❌ Kaputte Links:</span>
                      <strong className="text-red-400 text-2xl">{result.broken}</strong>
                    </div>
                    <div className="flex justify-between items-center text-lg">
                      <span className="text-gray-200 font-medium">⚠️ Weiterleitungen:</span>
                  ...
                  {result.broken > 0 && (
                    <div className="bg-red-900 border-2 border-red-600 rounded-lg p-4 mb-6">
                      <p className="text-xl font-bold text-white text-center">
                        💰 Geschätzter Umsatzverlust: ${result.estimatedLostRevenue}/Monat
                      </p>
                      <p className="text-sm text-red-200 text-center mt-2">
                        Basierend auf $60 Durchschnitts-Provision pro kaputtem Link
                      </p>
                    </div>
                  )}
                  ...
                  {result.brokenUrls && result.brokenUrls.length > 0 && (
                    <div className="mb-6">
                      <h4 className="font-bold text-red-400 mb-2">Kaputte Links:</h4>
                      <div className="bg-slate-900 rounded border-2 border-red-800 p-3 max-h-40 overflow-y-auto">
                        ...
                        {result.brokenUrls.length > 5 && (
                          <p className="text-sm text-gray-400 italic mt-2">
                            +{result.brokenUrls.length - 5} weitere...
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  >
                    Benachrichtigungen bei kaputten Links? Early Access sichern →
                  </a>
                </>
              )}
            </div>
